...
```{r tab3.1, results='asis', echo=FALSE, tab.cap="(ref:tab3.1)", message= F, warning= F}
db %>% dplyr::select(country, continent) %>% distinct(country, continent) %>%  
  kable(booktabs = T,col.names = c("País", "Continente")) %>% 
...
La literatura especializada sobre densidad sindical ha planteado que el ECM permite responder a las explicaciones de cambio(cf. Carruth & Disney, 1988; Disney, 1990), pero también de diferencias entre países (Schnabel, 2002), principalmente porque permite identificar una serie de especificaciones [^2]  de variables para distintas explicaciones teóricas (Keele et al., 2016)
